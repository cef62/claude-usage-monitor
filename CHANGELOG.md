...
- dee2b1b: macOS installs did not see the 0.14.0 update: its update feed listed only Windows. This release is offered to macOS again; the release build now runs the platforms one after the other and refuses to publish an incomplete feed.
...
- 6cb533f: Session and weekly percentages turn amber at your lowest alert threshold and red at the highest, in the popover and in the macOS menu bar. On by default; **Popover → Colored percentage** turns it off. The pace forecast line also gets a little more room below the history graph.

## 0.12.0

### Minor Changes

- d747670: Added a pace forecast: the session and weekly cards in the popover now show when you would reach 100% at your recent pace, or where you would land at the reset (Popover → "Pace forecast" hides it), and the new Alerts → "Run-out forecast" notification warns you once per window when a quota would run out well before it resets.
